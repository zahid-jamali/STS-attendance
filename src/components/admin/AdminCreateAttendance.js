...
const AdminCreateAttendance = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const entryDateTime = new Date(`${date}T${entryTime}`); // no Z suffix
    const exitDateTime = exitTime ? new Date(`${date}T${exitTime}`) : null;

    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URLS}/admin-creates-attendance`,
        {
          method: 'POST',
          headers: {
  ...
  const getUsers = async () => {
    try {
      let response = await fetch(
        `${process.env.REACT_APP_API_URLS}/getAllUsers`
      );
      if (response.ok) {
