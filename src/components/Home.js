...
const Home = () => {
  ...
  // Function to get user's projec
  const getProjects = async (userId) => {
    if (!userId) return;
    try {
      const req = await fetch(
        `${process.env.REACT_APP_API_URLS}/get-my-projects`,
        {
          method: 'POST',
  ...
  const markExit = async () => {
    if (!goal && !works) {
      return Swal.fire(
        'Warning',
        'Please select a project and describe your work.',
        'warning'
      );
    }
    try {
              ...
              {distance <= 50 ? (
                ...
                >
                  <h4 className="text-success">
                    ✅ You are at the required location
                  </h4>
                  <div className="d-flex flex-column align-items-center gap-3 mt-3">
                    ...
                    >
                      <option value="">Select the Goal</option>
                      {projects
                        ?.find((P) => P._id === selectedProject)
                        ?.Goals?.filter((G) => G.User === user._id)
                        ?.map((G) => (
                    ...
                    <textarea
                      ...
                      value={works}
                    ></textarea>
                    <button className="btn btn-danger w-100" onClick={markExit}>
                      Mark Exit
                    </button>
                  </div>
                </div>
